...
process_species_info <- function(species_list) {
  ...
  natserv_infos <- species_list |>
    ...
    suppressWarnings() |>
    dplyr::mutate(subnationCode = convert_province_codes(rlang::.data$subnationCode)) |>
  ...
  itis_common_names <- taxize::sci2comm(species_list, db = "itis", simplify = FALSE, messages = FALSE) |>
    ...
    dplyr::filter(rlang::.data$language %in% c("French", "English")) |>
    dplyr::mutate(commonName = stringr::str_replace_all(rlang::.data$commonName, "queue \u00E0 tache noire", "queue \u00E0 t\u00E2che noire")) |>
    dplyr::distinct() |>
    dplyr::mutate(commonName = stringr::str_to_sentence(rlang::.data$commonName)) |>
  ...
  col_infos <- taxize::gna_verifier(names = species_list, data_sources = 1) |>
    dplyr::mutate(col_link = paste0("https://www.catalogueoflife.org/data/taxon/", rlang::.data$currentRecordId)) |>
    dplyr::select(species = rlang::.data$submittedName, rlang::.data$col_link)
  ...
  final_species_df <- data.frame(species = species_list) |>
    ...
    dplyr::mutate(
      itis_url = paste0("https://www.itis.gov/servlet/SingleRpt/SingleRpt?search_topic=TSN&search_value=", rlang::.data$tsn),
      display_name = ifelse(!is.na(rlang::.data$French), rlang::.data$French, "Nom vernaculaire inconnu"),
      species_formatted = paste0(
        rlang::.data$display_name,
        " <br><small><em>", rlang::.data$species, "</em></small>",
        ifelse(!is.na(rlang::.data$gbif_url), paste0(" <a href='", rlang::.data$gbif_url, "' target='_blank' title='GBIF'>[GBIF]</a>"), ""),
        ifelse(!is.na(rlang::.data$col_link), paste0(" <a href='", rlang::.data$col_link, "' target='_blank' title='Catalogue of Life'>[CoL]</a>"), ""),
        ifelse(!is.na(rlang::.data$itis_url), paste0(" <a href='", rlang::.data$itis_url, "' target='_blank' title='ITIS'>[ITIS]</a>"), "")
      )
    )
