...
extraction-author: "`r params$extraction_author`"
citation-note: "`r params$citation_note`"
format:
  pdf:
...
La méthode d'extraction d'ADN environnemental est décrite dans Sandré et al. 2025. La moitié de chaque réplicat terrain est extraite et combinée pour former un réplicat technique à séquencer.
...
#| tbl-cap: "Nombre de lectures écartées par étape du pipeline Barque"
#| results: asis
...
format_with_pct <- function(value, pct) {
  paste0(
    "\\begin{tabular}[t]{@{}l@{}}",
...
for (sample_id in params$samples_ids) {
  ...
  brutes <- values[1]
  row_data <- c(
    format(brutes, big.mark = " "),
    sapply(2:length(values), function(i) {
      diff <- values[i-1] - values[i]
      pct <- round(diff / brutes * 100, 1)
      format_with_pct(values[i], pct)
    })
  )
...
total_row <- c("Total", format(total_brutes, big.mark = " "), sapply(2:length(total_values), function(i) {
  ...
  format_with_pct(total_values[i], pct)
}))
sequencing_data <- rbind(sequencing_data, total_row)

sequencing_table <- knitr::kable(
  sequencing_data,
  format = "latex",
  caption = NULL,
  booktabs = TRUE,
...
format_detection_cell <- function(label, reads) {
  paste0(
    "\\begin{tabular}[t]{@{}l@{}}",
    label, " \\\\ ",
    format(reads, big.mark = " "), " lectures assignées",
    "\\end{tabular}"
  )
}
...
classify_detection <- function(reads) {
  dplyr::case_when(
    reads > 100 ~ format_detection_cell("Confiant", reads),
    reads >= 10 ~ format_detection_cell("Probable", reads),
    TRUE ~ format_detection_cell("Incertain", reads)
  )
}
...
# Store ambiguous detections data for later (annexe)
ambiguous_data_list <- list()
...
for (sample_id in params$samples_ids) {
  ...
  # --- Store ambiguous detections data for annexe ---
  ...
  if (nrow(sample_multi) > 0) {
    # Store data for later printing in annexe
    ambiguous_data_list[[sample_id]] <- list(
      sample_id = sample_id,
...
# Annexe - Détections ambiguës

Les détections ambiguës représentent des séquences d'ADN qui correspondent à plusieurs espèces possibles. Ces détections sont présentées ici à titre informatif.

```{r}
#| label: annexe-ambiguous
#| results: asis

# Process and print ambiguous detections for each sample
for (sample_id in names(ambiguous_data_list)) {
  data <- ambiguous_data_list[[sample_id]]
  sample_multi <- data$sample_multi
  species_consolidated <- data$species_consolidated
  ...
  if (nrow(sample_multi) > 0) {
    ...
    sample_multi_final <- sample_multi_expanded |>
      ...
      dplyr::mutate(
        ...
        Groupe = dplyr::if_else(
          row_num == 1,
          paste0("\\multirow[t]{", n_species, "}{*}{\\textbf{Groupe ", group_id, "}}"),
          ""
        ),
        # Use multirow for Détection column
        Détection = dplyr::if_else(
          row_num == 1,
          paste0("\\multirow[t]{", n_species, "}{*}{", Statut, "}"),
          ""
        ),
        # Format species name
        Espèce = paste0(
    ...
    sample_multi_print <- sample_multi_final |>
      dplyr::select(Groupe, Détection, Espèce, `Statut Québec`, `Statut Canada`)

    # Place horizontal lines at group boundaries (before first row of each group and after last row)
    group_start_pos <- sort(unique(which(sample_multi_final$row_num == 1) - 1))
    group_end_pos <- sort(unique(which(sample_multi_final$row_num == sample_multi_final$n_species)))
    add_lines_pos <- list(group_start_pos, group_end_pos)
    add_lines_cmd <- c("\\hline\n", "\\hline\\addlinespace[4pt]\n")

    tbl <- knitr::kable(
      sample_multi_print,
      format = "latex",
      caption = paste0("Détections ambiguës dans l'échantillon ", sample_id),
      label = NA,
      escape = FALSE,
      booktabs = FALSE,
      longtable = TRUE,
      ...
      kableExtra::row_spec(seq_len(nrow(sample_multi_print))[-1], extra_latex_after = "\\cline{3-5}")
