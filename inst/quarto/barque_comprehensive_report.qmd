    ...
    bslib::card(
        bslib::card_body(
            bslib::value_box(
                ...
                htmltools::p("Bases séquencées ayant un score de Phred d'au moins 30)")
            )
        )
    ),
    bslib::card(
        bslib::card_body(
            bslib::value_box(
                ...
                htmltools::p("Score de qualité Phred moyen (PF - Passing Filter)")
            )
        )
    )
)
...
bslib::layout_columns(
    ...
    bslib::card(
        bslib::card_body(
            bslib::value_box(
                title = toupper("Échantillons analysés"),
                value = stats$samples,
                showcase = bsicons::bs_icon("grid-3x3-gap"),
                htmltools::p("Nombre total d'échantillons dans l'analyse")
            )
        )
    ),
    bslib::card(
        bslib::card_body(
            bslib::value_box(
                title = toupper("Espèces détectées"),
                value = stats$single_species,
                showcase = bsicons::bs_icon("bug"),
                htmltools::p("Espèces uniques identifiées")
            )
        )
    )
)
    ...
    bslib::card(
        bslib::card_body(
            bslib::value_box(
                ...
                htmltools::p("Assignations multi-espèces")
            )
        )
    ),
    bslib::card(
        bslib::card_body(
            bslib::value_box(
                ...
                htmltools::p("Lectures de séquences combinées")
            )
        )
    )
)
...
bslib::layout_columns(
    ...
    bslib::card(
        bslib::card_body(
            bslib::value_box(
                title = toupper("Lectures espèce unique"),
    ...
    bslib::card(
        bslib::card_body(
            bslib::value_box(
                title = toupper("Lectures multi-espèces"),
                value = htmltools::HTML(paste0(
...
Nombre de lectures écartés lors du post-traitement des données de séquencage par le pipeline barque.
...
dropout_long <- barqueReport::load_dropout_data(params$barque_output_folder, params$samples_ids) |>
    dplyr::mutate(
        step_label = dplyr::case_when(
            step == "04_data" ~ "Nombre de séquences brutes",
            step == "05_trimmed" ~ "Filtration des séquences",
            step == "06_merged" ~ "Fusion des séquences",
            step == "07_split_amplicons" ~ "Classifications des séquences",
            step == "08_chimeras" ~ "Retrait des chimères",
            step == "12_results" ~ "Nombre de séquences finales",
            TRUE ~ step
        ),
...
Visualisation en carte de chaleur du nombre de lectures par espèce à travers les échantillons.
...
## Présence des espèces dans chaque province 

Pour chaque espèce, nous avons interrogé la base de données [NatureServe Canada](https://www.natureserve.org/) pour savoir si elle a déjà été observé dans chacune des provinces canadiennes. 
...
dplyr::select(single_hits, Group, Species) |>
    ...
    dplyr::select(-Species, -gbif_url, -col_link, -itis_url, -display_name, -tsn, -English, -French, -exotic, -native, -value) |>
    reactable::reactable(
        # Enhanced default styling
        defaultColDef = reactable::colDef(
            sortable = TRUE,
